...
export default function Generator({ scenes }: { scenes: string[] }) {
  ...
  const libGallery = useMemo(() => (mode === "scene" ? scenes : LIB_MODELS), [mode, scenes]);
  ...
  async function onGenerate() {
    ...
    try {
      const res = await fetch("/api/generate", {
        ...
        body: JSON.stringify({ mode, userImageBase64: userImg, libImageBase64: libImg, preset, prompt }),
      });
      ...
      <div className="max-w-5xl mx-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section className="space-y-2">
            <h2 className="font-medium">1) Наш ассет ({mode === "scene" ? "фон/локация" : "модель/одежда"})</h2>
          ...
          <section className="space-y-3">
            <h2 className="font-medium">2) Фото пользователя</h2>
          ...
          <section className="space-y-2 md:col-span-2">
            ...
            {result ? (
              <div className="space-y-2">
                ...
                <div>
                  <button onClick={download} className="px-4 py-2 rounded-xl border">Скачать</button>
                </div>
              </div>
            ) : (
